...
impl ItemEnum {
    pub fn generics(&self) -> Option<&Generics> {
        Some(match *self {
            ...
            ItemEnum::TypedefItem(ref t) => &t.generics,
    ...
    fn clean(&self, cx: &DocContext) -> Item {
        let inner = match self.node {
            ...
            hir::ImplItemKind::Type(ref ty) => {
                AssociatedTypeItem(Vec::new(), Some(ty.clean(cx)))
            },
        };
        Item {
            name: Some(self.name.clean(cx)),
    ...
    fn clean(&self, cx: &DocContext) -> Item {
        let inner = match self.kind {
            ...
            ty::AssociatedKind::Type => {
                ...
                let (bounds, ty) = if let ty::TraitContainer(did) = self.container {
                    ...
                    (Vec::new(), Some(cx.tcx.type_of(self.def_id)))
                };
...
fn build_deref_target_impls(cx: &DocContext,
    ...
    for item in items {
        let target = match item.inner {
            AssociatedTypeItem(_, Some(ref t)) => t,
            _ => continue,
        };
