        ...
        if let Some(impls) = c.impls.get(&did) {
            for i in impls {
                ...
                if impl_.trait_.def_id().and_then(|d| c.traits.get(&d))
                                        .map_or(false, |i| if let clean::TraitItem(ref t) = i.inner { t.is_spotlight } else { false }) {
                    if out.is_empty() {
                        out.push_str(
...
fn render_impl(w: &mut fmt::Formatter, cx: &Context, i: &Impl, link: AssocItemLink,
    ...
    if render_mode == RenderMode::Normal {
        let id = derive_id(match inner_impl.trait_ {
    ...
    if let Some(t) = trait_ {
        for trait_item in &t.items {
            ...
            doc_impl_item(w, cx, trait_item, assoc_link, render_mode, true,
                          i.impl_item.stable_since(), None, show_def_docs)?;
        }
    }
    write!(w, "</div>")?;
