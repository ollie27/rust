...
impl FileEntry {
    fn parse_ids(&mut self, file: &Path, contents: &str, errors: &mut bool) {
        if let FileEntry::File { ref mut ids, .. } = *self {
            if ids.is_empty() {
    ...
    for entry in t!(dir.read_dir()).map(|e| t!(e)) {
        ...
        if kind.is_dir() {
            ...
            if let Some(pretty_path) = pretty_path {
                ...
                if let FileEntry::File { ref mut source, .. } = *cache.get_mut(&pretty_path).unwrap() {
                    *source = String::new();
                }
            }
        }
...
fn check(cache: &mut Cache,
    ...
    with_attrs_in_source(&contents, " href", |url, i, base| {
        ...
        let (pretty_path, contents) = match load_file(cache, root, &path, FromRedirect(false)) {
            Ok(res) => res,
            Err(LoadError::IOError(_err)) => {
        ...
        if let Some(ref fragment) = fragment {
            ...
            if let FileEntry::File { ref ids, .. } = *entry {
    ...
    Some(pretty_file.to_owned())
}

fn load_file<'a>(cache: &'a mut Cache,
             root: &Path,
             file: &Path,
             redirect: Redirect)
             -> Result<(&'a Path, String), LoadError> {
    ...
    let maybe_redirect = if cache.contains_key(pretty_file) {
        match cache[pretty_file] {
            FileEntry::File { ref source, .. } => {
                if !contents.is_empty() {
                    println!("cloning occupied source of {}, len: {}", pretty_file.display(), contents.len());
                }
                contents = source.clone();
                None
            }
            FileEntry::Redirect { ref dest } => {
        ...
        // println!("read file from disk: {}", file.display());
        fp.read_to_string(&mut contents).map_err(|err| LoadError::IOError(err))?;

        let maybe = maybe_redirect(&contents);
        if let Some(url) = maybe {
            if let SkipRedirect = redirect {
                return Err(LoadError::IsRedirect);
            }
            cache.insert(pretty_file.to_owned(), FileEntry::Redirect {
                dest: url.clone(),
            });
            Some(url)
        } else {
            if !contents.is_empty() {
                println!("cloning vacant source of {}, len: {}", pretty_file.display(), contents.len());
            }
            cache.insert(pretty_file.to_owned(), FileEntry::File {
    ...
    match maybe_redirect.map(|url| file.parent().unwrap().join(url)) {
        ...
        None => Ok((cache.get_key_value(pretty_file).unwrap().0, contents)),
    }
}
